...
public class BreedView extends VerticalLayout {
  ...
  private Grid<Breed> create_Grid() {
    ...
    define_GridLineDetailsRow(grid);


    return grid;
  }

  private void define_GridLineDetailsRow(Grid<Breed> grid) {
    ...
    create_checkDetailsButton(grid, false, "Check Details");

    grid.addThemeVariants(GridVariant.LUMO_ROW_STRIPES);
    grid.setSelectionMode(Grid.SelectionMode.NONE);
  }

  private static void create_checkDetailsButton(
       Grid<Breed> grid,
       boolean detailsVisible,
       String buttonText) {

    grid.setDetailsVisibleOnClick(detailsVisible);
